...
def _render_scene_ffmpeg(
    ...
    Effects alternate by scene index:
      - Even scenes (0,2,4): zoom OUT 1.25→1.0 from center (reveals full image)
      - Odd scenes (1,3,5):  zoom IN  1.0→1.25 from center (crops into center)
    ...
    if scene_idx % 2 == 0:
        zoom_expr = f"if(eq(on\\,1)\\,1.25\\,max(1.0\\,zoom-0.25/{safe_frames}))"
    else:
        zoom_expr = f"min(1.25\\,1.0+0.25*(on-1)/{safe_frames})"
